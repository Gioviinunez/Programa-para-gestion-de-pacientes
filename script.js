...
document.getElementById('delete-expense').addEventListener('click', function() {
    const expenseDescription = prompt('Ingrese la descripción del gasto a borrar:');
    if (expenseDescription) {
        // Buscar el gasto por la descripción y eliminarlo
        const expenseIndex = expenses.findIndex(expense => expense.description === expenseDescription);
        if (expenseIndex !== -1) {
            ...
            alert(`El gasto "${expenseDescription}" ha sido borrado.`);
        } else {
            alert(`No se encontró un gasto con la descripción: "${expenseDescription}".`);
        }
    }
});
